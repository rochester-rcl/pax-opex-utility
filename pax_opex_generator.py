...
preing_projfol_frame = sg.Frame('Project Folder', [
    ...
    [sg.Push(), sg.Text('Generate File/Hash Manifest?'), sg.Checkbox('', default=True, key='-MANIFEST-'), sg.Combo(['', 'MD5', 'SHA-1'], default_value=sg.user_settings_get_entry('algorithm', default=''), key='-ALG-', readonly=True), sg.Push()]], expand_x=True, pad=5)
...
ops_col = sg.TabGroup([[tab1, tab2, tab3]], pad=5, key='-TAB_GROUP-')

settings_frame = sg.Frame('Settings',
                    [[sg.Button('Save', font=(button_font), pad=5), sg.Button('Display', font=(button_font), pad=5)]], pad=5)

output_col = sg.Column([[sg.Output(key='-OUTPUT-', size = (60, 27), background_color='#252525', expand_x=True, pad=5)],
                [settings_frame, sg.Button('Help', font=(button_font), pad=5), sg.Button('Clear', font=(button_font), pad=5), sg.Button('About', font=(button_font), pad=5), sg.Push(), sg.Button('Exit', font=(button_font), pad=5), sg.Sizegrip()]], expand_x=True, vertical_alignment='t')
